#include <bits/stdc++.h>
#include <iostream>
using namespace std;
#define mp make_pair
#define pb push_back
#define f first
#define s second
#define MAX_INT 2147483647
#define MAX_LONG 9223372036854775807
#define MAX_DOUBLE 1e18
typedef long long int ll;
#define MAXN 200000
#define MAXST (1<<19)
int M, N, Q, stLen, minPow;
ll st[MAXST+1];
ll lzy[MAXST+1];
void buildST() {
    minPow = N-1; int cnt = 0;
    while(minPow != 0) {
        minPow >>= 1;
        cnt++;
    }
    minPow = 1<<cnt;
    stLen = minPow*2-1;
    for(int i = minPow; i < minPow+N; i++) {cin >> st[i];}
    for(int i = minPow-1; i > 0; i--) {st[i] = st[2*i]+st[2*i+1];}
}
void propagate(int idx, int s, int e) {
    if(lzy[idx] != 0 && s != e) {
        lzy[idx*2] += lzy[idx];
        st[idx*2] += lzy[idx]*((e-s+1)/2);
        lzy[idx*2+1] += lzy[idx];
        st[idx*2+1] += lzy[idx]*((e-s+1)/2);
        lzy[idx] = 0;
    }
}
ll query(int idx, int s, int e, int l, int r) {
    if(e < l || s > r) return 0;
    propagate(idx,s,e);
    if(s >= l && e <= r) return st[idx];
    int mid = (s+e)/2;
    return ((mid >= l)?query(idx*2, s, mid, l, r):0) + ((mid <= r)?query(idx*2+1, mid+1, e, l, r):0);
}
void update(int idx, int s, int e, int l, int r, ll val) {
    if(e < l || s > r) return;
    propagate(idx,s,e);
    if(s >= l && e <= r) {
        st[idx] += val*(e-s+1);
        lzy[idx] += val;
        return;
    }
    int mid = (s+e)/2;
    if(mid >= l) update(idx*2, s, mid, l, r, val);
    if(mid <= r) update(idx*2+1, mid+1, e, l, r, val);
    st[idx] = st[idx*2]+st[idx*2+1];
}
int main() {
    ios_base::sync_with_stdio(0); cin.tie(0);
    memset(st, 0, sizeof(st));
    memset(lzy, 0, sizeof(lzy));
    cin >> M >> N >> Q;
    buildST();
    for(int i = 0,t,l,r,x; i < Q; i++) {
        cin >> t;
        if(t == 1) {
            cin >> l >> r >> x;
            update(1,1,minPow,l,r,x);
        } else {
            cin >> l >> r;
            cout << query(1,1,minPow,l,r)%M << "\n";
        }
    }
    return 0;
}